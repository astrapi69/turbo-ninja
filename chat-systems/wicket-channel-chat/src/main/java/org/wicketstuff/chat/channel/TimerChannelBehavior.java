 ...
 * <p>
 * The polling interval is configured in the constructor. The more frequent is the polling, the more
 * quickly your client will be updated, but also the more you will load your server and your
 * network.
 * <p>
 * A timeout can also be configured to indicate when the behavior should consider the page has been
 * disconnected. This is important to clean appropriately the resources associated with the page.
 * 
 * @author Xavier Hanin
 * 
 */
public class TimerChannelBehavior extends AbstractAjaxTimerBehavior implements Serializable
{
	 ...
	 * 
	 * The method calls are actually calls to methods on {@link AjaxRequestTarget}, which are
	...
	private static class DelayedMethodCallList implements Serializable
	{
		...
		private class DelayedMethodCall implements Serializable
		{
			...
			/**
			 * The index of the method to invoke We store only an index to avoid serialization
			 * issues
			 */
			private final int m;
			/**
	...
	/**
	 * An {@link IPushTarget} implementation which enqueue {@link DelayedMethodCallList}, also
	 * called triggers, for a {@link TimerChannelBehavior} identified by its id.
	 * 
	...
	{
		/**
		 * A trigger currently being constructed, waiting for a call to trigger to go to the
		 * triggers list.
		 */
		...
		/**
		 * The duration to wait before considering that a page is not connected any more This is
		 * usually set to the polling interval + a safety margin
		 */
	...
	/**
	 * Returns the page behavior id corresponding the given behavior id. Only one behavior is
	 * actually rendered on a page for the same updateInterval, to optimize the number of requests.
	 * Therefore all timer channel behaviors of the same page are redirected to the same id, using
	 * this method.
	 * 
	 * @param application
	 *            the wicket application to which the behavior belong
	 * @param id
	 *            the id of the behavior for which the page behavior id should be found
	...
	public static boolean isConnected(final Application application, final String id,
		final Duration timeout)
	{
		...
		final String redirectedId)
	{
		...
		if (triggersById != null)
		{
			...
			if (triggersToRedirect != null)
			{
				...
				{
					final List<DelayedMethodCallList> triggers = getTriggers(application,
						redirectedId);
					synchronized (triggers)
					{
		...
		if (eventsTimeById != null)
		{
			...
			/*
			 * we don't need to merge touch information, since merged behaviors always have the same
			 * touch rates
			 */
		}
	}

	private static void setRedirectId(final Application application, final String id,
		final String redirectedId)
	{
		ConcurrentMap<String, String> pageIdsById;
		synchronized (application)
		...
		{
			/*
			 * The id was not already redirected to the redirectedId, we need to merge the
			 * information before redirection with information after redirection
			 */
	...
	{
		touch(getComponent().getApplication(), id);
		final List<DelayedMethodCallList> triggers = getTriggers(getComponent().getApplication(),
			id);
	...
	public void renderHead(final Component component, final IHeaderResponse response)
	{
		...
		{
			/*
			 * A similar behavior has already been rendered, we have no need to render ourself All
			 * we need is redirect our own behavior id to the id of the behavior which has been
			 * rendered.
			 */
			final String redirectedId = getPageId(getComponent().getApplication(),
				timerChannelPageId);
