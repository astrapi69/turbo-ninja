...
public class TestComponentExpression {
	...
	public void testNormalExpressionWithSubType() {
		...
		assertEquals(1, ComponentExpression.findAllComponents(parent,
				"one1:two1", WebMarkupContainer.class).size());
		assertEquals(1, ComponentExpression.findAllComponents(parent,
				"one1:two1", WMCSubClass.class).size());
	}

	@Test
	public void testStarExpressionWithSubType() {

		assertEquals(2, ComponentExpression.findAllComponents(parent, "one1:*",
				WebMarkupContainer.class).size());
		assertEquals(1, ComponentExpression.findAllComponents(parent, "one1:*",
				WMCSubClass.class).size());
	...
	public void testStarStarExpression1WithSubType() {
		...
		assertEquals(5, ComponentExpression.findAllComponents(parent, "**",
				WebMarkupContainer.class).size());
		assertEquals(2, ComponentExpression.findAllComponents(parent, "**",
				WMCSubClass.class).size());
	...
	public void testStarStarExpression2WithSubType() {
		assertEquals(1, ComponentExpression.findAllComponents(parent,
				"one1:**:three1", WebMarkupContainer.class).size());
		assertEquals(0, ComponentExpression.findAllComponents(parent,
				"one1:**:three1", WMCSubClass.class).size());
	}

	@Test
	public void testStarStarExpression3WithSubType() {
		assertEquals(0, ComponentExpression.findAllComponents(parent,
				"**:two1:**:four1", WebMarkupContainer.class).size());
		assertEquals(1, ComponentExpression.findAllComponents(parent,
				"**:two2:**:four1", WebMarkupContainer.class).size());

		assertEquals(0, ComponentExpression.findAllComponents(parent,
				"**:two1:**:four1", WMCSubClass.class).size());
		assertEquals(1, ComponentExpression.findAllComponents(parent,
				"**:two2:**:four1", WMCSubClass.class).size());
	...
	public void testNormalExpression() {
		...
		assertEquals(two1, ComponentExpression.findComponent(parent,
				"one1:two1"));
		assertEquals(1, ComponentExpression.findAllComponents(parent,
				"one1:two1").size());

		assertEquals(three1, ComponentExpression.findComponent(parent,
				"one1:two2:three1"));
		assertEquals(1, ComponentExpression.findAllComponents(parent,
				"one1:two2:three1").size());
	...
	public void testStarExpression() {
		...
		assertEquals(2, ComponentExpression.findAllComponents(parent, "one1:*")
				.size());
		assertEquals(two1, ComponentExpression.findAllComponents(parent,
				"one1:*").get(0));
		assertEquals(two2, ComponentExpression.findAllComponents(parent,
				"one1:*").get(1));
	...
	public void testStarStarExpression1() {
		...
		assertEquals(two1, ComponentExpression.findComponent(parent, "one1:**"));
		assertEquals(4, ComponentExpression
				.findAllComponents(parent, "one1:**").size());
		assertEquals(two1, ComponentExpression.findAllComponents(parent,
				"one1:**").get(0));
		assertEquals(two2, ComponentExpression.findAllComponents(parent,
				"one1:**").get(1));
		assertEquals(three1, ComponentExpression.findAllComponents(parent,
				"one1:**").get(2));
		assertEquals(four1, ComponentExpression.findAllComponents(parent,
				"one1:**").get(3));
	}
	...
	public void testStarStarExpression2() {
		...
		assertEquals(1, ComponentExpression.findAllComponents(parent,
				"one1:**:four1").size());
		assertEquals(four1, ComponentExpression.findComponent(parent,
				"one1:**:four1"));
	}
